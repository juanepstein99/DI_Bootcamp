...
def get_words_from_file(file_path):
    with open (file_path, "r", encoding="utf-8") as word_file:
        all_the_file = word_file.read()
        word_list = all_the_file.split()
        return word_list
    
# Step 2: Create the get_random_sentence function
def get_random_sentence(length):
    word_list = get_words_from_file(WORDS_FILE)
    sentence_words = []
    for word in range(length):
        sentence_words.append(random.choice(word_list))
...
def main():
    ...
    user_input = input("Give me a lenght for the sentence")
    try: 
...
# Step 3: Add the “birth_date” key
data["company"]["employee"]["birth_date"] = "1999-4-26"
